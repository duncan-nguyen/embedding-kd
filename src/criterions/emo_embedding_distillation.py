...
def sinkhorn_batched(
    ...
    # The primal scaling vectors can overflow even when the final plan is finite:
    # a very small marginal divided by a very small kernel product makes one
    # vector huge and the other tiny.  The resulting ``inf * 0`` poisons both the
    # loss and its gradient.  Updating the same Sinkhorn equations in log space
    # keeps the intermediate representation finite on real EMO activations.
    # A finite log-zero is deliberate.  ``logsumexp`` over an all-``-inf``
    # padded row has an undefined backward (all softmax terms are ``0 / 0``),
    # even when that output is masked immediately afterwards.
    log_zero = -1e4
    log_kernel = (-cost_matrix / alpha).masked_fill(~keep, log_zero)
    a = a.squeeze(2)
    b = b.squeeze(2)
    positive_a = valid_a & (a > 0)
    positive_b = valid_b & (b > 0)
    log_a = a.clamp_min(eps).log().masked_fill(~positive_a, log_zero)
    log_b = b.clamp_min(eps).log().masked_fill(~positive_b, log_zero)
    log_u = torch.zeros(batch, m, device=device, dtype=dtype).masked_fill(
        ~positive_a, log_zero
    )
    log_v = torch.zeros(batch, n, device=device, dtype=dtype).masked_fill(
        ~positive_b, log_zero
    )

    for iteration in range(max_iter):
        previous = log_u
        denominator_v = torch.logsumexp(
            log_kernel + log_u.unsqueeze(2), dim=1
        ).masked_fill(~positive_b, 0.0)
        log_v = (log_b - denominator_v).masked_fill(~positive_b, log_zero)
        denominator_u = torch.logsumexp(
            log_kernel + log_v.unsqueeze(1), dim=2
        ).masked_fill(~positive_a, 0.0)
        log_u = (log_a - denominator_u).masked_fill(~positive_a, log_zero)
        if (iteration + 1) % check_every == 0:
            current_finite = log_u.masked_fill(~positive_a, 0.0)
            previous_finite = previous.masked_fill(~positive_a, 0.0)
            if (current_finite - previous_finite).abs().amax() < stop_thr:
                break
...
def compute_att_loss_2(
    ...
    *,
    ...
    weights = contributes.to(device=device, dtype=torch.float64)
