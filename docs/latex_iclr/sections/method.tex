...
\mathbb{S}^{d_S-1}$. GATE-KD primarily transfers teacher knowledge through an
...
endpoint loss, while leaving the $H_0$ regularizer unchanged.
\end{corollary}
\begin{proof}
...
gauge. The $H_0$ regularizer depends only on the fixed native teacher and
student embeddings and is independent of $R$.
\end{proof}

\subsection{Native-Space Topological Regularization}
...
embeddings. We therefore add a structural regularizer defined directly in the
teacher's native space.
...
$\operatorname{rank}(T)>d_S$. This relaxation ranges over arbitrary
rank-$d_S$ Gram approximations and need not be tight under the student's
positive-semidefinite and unit-diagonal constraints.
\end{proposition}
\begin{proof}
...
connectivity scales
\begin{equation}
    \nu_S=\frac{1}{B-1}\sum_{j=1}^{B-1}\delta_{q_{(j)}^S},
    \qquad
    \nu_T=\frac{1}{B-1}\sum_{j=1}^{B-1}\delta_{q_{(j)}^T},
\end{equation}
