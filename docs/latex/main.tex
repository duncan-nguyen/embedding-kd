...
embeddings of the same sentences. Since $R$ is orthogonal, the Gram matrix of the targets, the retained
variance and the
...
interacts with student optimization. Because Eq.~\eqref{eq:gauge} is the
exact minimizer over $O(d_S)$ for a fixed student, $R$ may also be
re-estimated between epochs against the current student; the resulting
alternation is a coordinate descent on
$\min_{\theta,R}\mathcal{E}_{\mathrm{sem}}(\mathbf{Z}_\theta,\mathbf{T}R)$
that cannot increase the objective and, $R$ being orthogonal, never alters
the target geometry. We report the fixed initial gauge as the default and
the re-estimated gauge as an ablation.
...
\begin{equation}
    ...
    \qquad
    \widetilde{\boldsymbol{\tau}}_i=\operatorname{norm}\bigl(f_T(x_i)\bigr),
    \label{eq:gram}
\end{equation}
whose $(i,j)$-th entries are pairwise cosine similarities. Note that
$\mathbf{G}_T$ is built from the teacher's \emph{native} embeddings, not
from the projected targets of Eq.~\eqref{eq:teacher_embedding}: a cosine
matrix is dimension-free, so the relational structure of the teacher can be
transferred without any loss, and the projection $P_T$ enters only the
point-wise term where a coordinate system is unavoidable. In our setting the
$7\%$ of embedding energy that $P_T$ discards therefore affects the anchor
$\boldsymbol{\tau}_i$ but not the geometry the student is asked to
reproduce.
...
\For{each training mini-batch $\mathcal{B}$}
    ...
    \For{$\ell=1,\ldots,L-1$}
        ...
        \gets
        \widetilde{\mathbf{T}}\widetilde{\mathbf{T}}^\top$
        from the native teacher embeddings, Eq.~\eqref{eq:gram}
