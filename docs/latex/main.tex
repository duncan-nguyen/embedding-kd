...
dimension using a fixed linear dimensionality reduction map
$P_T\in\mathbb{R}^{d_T\times d_S}$. Our default choice is PCA fitted only
on the cached training embeddings:
\begin{equation}
    \boldsymbol{\tau}_i
    =
    \operatorname{norm}
...
cached embedding energy. $P_T$ is fitted once and frozen; it never
interacts with student optimization and is discarded at inference.
...
\State{} Fit fixed PCA map $P_T$ on cached teacher embeddings
...
\begin{equation}
    ...
    \mathbf{z}^{(L)}.
\end{equation}
No PCA target projection, Gram matrix, semantic energy, vector field,
